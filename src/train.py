...
def train_model(data_dir: str,
    ...
    print("🚀 Phase 1: Training frozen model...")
    history = model.fit(
        train_ds,
    ...
    # Fine-tuning (Phase 2: Unfrozen)
    print("🔓 Phase 2: Fine-tuning...")
    from model import unfreeze_model
    model = unfreeze_model(model)
    
    # Train again with low LR
    history_finetune = model.fit(
        train_ds,
        validation_data=val_ds,
        initial_epoch=history.epoch[-1],
        epochs=epochs + 10,  # Add 10 more epochs
        class_weight=class_weights,
        callbacks=callbacks
    )
    
    # Merge histories (optional, but good for returning)
